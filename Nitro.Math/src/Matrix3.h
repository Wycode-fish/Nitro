...
namespace Nitro
{
	namespace Math
	{
		...
		public:
            ...
            INLINE explicit Matrix3(const FMAT4& m) { m_mat[0] = Vector3(m.r[0]); m_mat[1] = Vector3(m.r[1]); m_mat[2] = Vector3(m.r[2]); }
            ...
            INLINE operator FMAT4() const { return (const FMAT4&)m_mat; }

            INLINE Vector3 operator* (Vector3 vec) const { return Vector3(XMVector3TransformNormal(vec, *this)); }
            INLINE Matrix3 operator* (const Matrix3& mat) const { return Matrix3(*this * mat.m_mat[0], *this * mat.m_mat[1], *this * mat.m_mat[2]); }
		private:
			Vector3 m_mat[3];
		};
	}
